...
/**
 * <h4>MarketingDashboardController - Bộ điều khiển bảng điều khiển
 * marketing</h4>
 *
 * <p>
 * Servlet này chịu trách nhiệm hiển thị bảng điều khiển (dashboard) cho vai trò
 * Marketing, cho phép người dùng lọc theo khoảng thời gian, thống kê về doanh
 * thu, đơn hàng, khóa học, môn học và tài khoản.</p>
 *
 * <p>
 * Các số liệu thống kê được lấy từ cơ sở dữ liệu, so sánh với khoảng thời gian
 * trước đó và hiển thị trên giao diện theo dạng biểu đồ hoặc bảng số liệu.</p>
 *
 * @author HuongNI
 */
@WebServlet(name = "MarketingDashboardController", urlPatterns = {"/admin/dashboard"})
public class MarketingDashboardController extends HttpServlet {
    ...
    /**
     * <h4>doGet - Xử lý yêu cầu hiển thị dashboard, redirect to homepage if not
     * admin or marketer</h4>
     *
     * <p>
     * Phân tích `startDate` và `endDate` từ request, mặc định là 7 ngày gần
     * nhất nếu không có. Sau đó gọi các hàm phụ để chuẩn bị dữ liệu thống kê và
     * forward sang giao diện JSP.</p>
     *
     ...
     *
     * <p>
     * Được sử dụng khi xảy ra lỗi định dạng ngày hoặc lỗi logic (ví dụ:
     * startDate > endDate), đồng thời set thuộc tính để hiển thị thông báo lỗi
     * trên giao diện.</p>
     *
     * @param request yêu cầu HTTP
     * @param response phản hồi HTTP
     ...
     *
     * <p>
     * Phân tích khoảng thời gian hiện tại, lấy các dữ liệu thống kê tương ứng
     * từ DAO, bao gồm môn học, khóa học, tài khoản, doanh thu, và xu hướng đơn
     * hàng.</p>
     *
     * @param request yêu cầu HTTP
     ...
     *
     * <p>
     * Tính phần trăm thay đổi giữa giá trị trước và hiện tại. Trả về 100 nếu
     * giá trị cũ bằng 0 và giá trị mới khác 0.</p>
     *
     ...
     *
     * <p>
     * Lấy số lượng môn học mới và tổng số môn học trong thời gian hiện tại, so
     * sánh với khoảng thời gian trước đó để tính phần trăm thay đổi.</p>
     */
     ...
     *
     * <p>
     * Phân tích số lượng đơn hàng có trạng thái: PAID, DECLINED, SENT trong
     * thời gian hiện tại và trước đó, tính phần trăm thay đổi.</p>
     */
     ...
     *
     * <p>
     * Đếm số lượng tài khoản mới được tạo và số người mới có đơn hàng trong
     * thời gian chỉ định, tính phần trăm thay đổi so với giai đoạn trước.</p>
     */
     ...
     *
     * <p>
     * Tổng hợp doanh thu theo danh mục môn học trong khoảng thời gian hiện tại,
     ...
     *
     * <p>
     * Dựa vào độ dài khoảng thời gian, phân tích theo ngày, tuần hoặc tháng.
     * Kết quả bao gồm danh sách số lượng đơn hàng đã thanh toán và tổng đơn
     * hàng từng mốc thời gian.</p>
     */
     ...
     *
     * <p>
     * Thêm dữ liệu từng ngày từ `start` đến `end` vào danh sách thống kê.</p>
     */
     ...
     *
     * <p>
     * Gộp dữ liệu thành các tuần 7 ngày để hiển thị xu hướng.</p>
     */
     ...
     *
     * <p>
     * Lấy dữ liệu theo tháng (từ ngày 1 đến cuối tháng) và tính tổng số đơn
     * hàng.</p>
     */
