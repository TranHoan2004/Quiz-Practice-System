...
<div class="card shadow-lg">
    ...
    <div class="card-body p-4">
        <form id="questionForm" action="question-details" method="POST">
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="dimensionSelect" name="dimensionId">
                        <c:forEach var="dimension" items="${dimensions}">
                            <option value="${dimension.id}" ${requestScope.dimensionCurrent.id == dimension.id ? 'selected' : ''}>
                                    ${dimension.value}
                            </option>
                        </c:forEach>
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="statusSelect" name="status">
                        <option value="true" ${requestScope.question.status ? 'selected' : ''}>Published</option>
                        <option value="false" ${!requestScope.question.status ? 'selected' : ''}>Draft</option>
                    </select>
                </div>
            </div>
                    ...
                    <div class="media-item mb-3 p-3 border rounded shadow-sm">
                        <div class="flex-grow-1">
                            ...
                            <input type="text" class="form-control mb-2" name="mediaUrl_${media.id}"
                                   value="${media.link}" placeholder="Media URL">
                            <textarea class="form-control" rows="2" placeholder="Enter the note/caption..."
                                      name="mediaCaption_${media.id}">${media.note}</textarea>
                        </div>
                        <div class="d-flex flex-column gap-2">
                            <a href="${media.link}" target="_blank" class="btn btn-outline-info btn-sm"><i
                            ...
                            <div class="option-item mb-3 p-3 border rounded shadow-sm">
                                <div class="form-check me-3">
                                    <input class="form-check-input" type="radio" name="correctOption"
                                           ...
                                           value="${option.id}" ${option.isTrue ? 'checked' : ''}>
            ...
            <div class="d-flex justify-content-end gap-2">
                ...
                <button type="button" class="btn btn-secondary btn-lg"><i class="fas fa-times"></i> Cancel</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('addOptionBtn').addEventListener('click', function () {
            ...
            const newOptionItem = `
                        <div class="option-item mb-3 p-3 border rounded shadow-sm">
                            <div class="form-check me-3">
                                <input class="form-check-input" type="radio" name="correctOption" id="option${newOptionId}isTrue" value="${newOptionId}">
    ...
    async function handleSubmit(event) {
        ...
        const dimensionSelect = document.getElementById('dimensionSelect').value;
        ...
        try {
            const res = await fetch(`${pageContext.request.contextPath}/question-details`, {
                method: 'POST',
                headers: {
