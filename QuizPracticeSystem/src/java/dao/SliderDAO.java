...
public class SliderDAO extends DBContext {
    ...
    public List<Slider> getTopSliderActive(int limit) {
        ...
        var sql = "SELECT * FROM `swp391`.slider s WHERE s.status = ? ORDER BY RAND() LIMIT ?";
    ...
    public List<Slider> getAllSliders() {
        var sql = "SELECT * FROM `swp391`.slider";
        List<Slider> list = new ArrayList<>();
        try (var connection = getConnection();
             var ps = connection.prepareStatement(sql);
    ...
    public List<Slider> search(String keyword) {
        var sql = "SELECT * FROM `swp391`.slider WHERE title LIKE ?";
        List<Slider> list = new ArrayList<>();
        try (var connection = getConnection();
             var ps = connection.prepareStatement(sql)) {
    ...
    public List<Slider> filterByStatus(String status) {
        var sql = "SELECT * FROM `swp391`.slider WHERE status = ?";
        List<Slider> list = new ArrayList<>();
        try (var connection = getConnection();
             var ps = connection.prepareStatement(sql)) {
    ...
    public Slider getSliderById(UUID id) {
        var sql = "SELECT * FROM `swp391`.slider WHERE id = ?";
        Slider slider = null;
        try (var connection = getConnection();
             var ps = connection.prepareStatement(sql)) {
    ...
    public void updateSliderStatus(UUID id, boolean status) {
        var sql = "UPDATE `swp391`.slider SET status = ? WHERE id = ?";
        try (var connection = getConnection();
             var ps = connection.prepareStatement(sql)) {
